...
public class _3011_xBC3_ROOM_REQUEST_MESSAGE_ROOM extends HandlerBase {

    public _3011_xBC3_ROOM_REQUEST_MESSAGE_ROOM(DanceServer server) {
        super(server);
    }
    ...
    public SendPacket[] handle(ReadPacket packet, DanceClient danceClient) {
        ...
        ChatMessage chatMessage = new ChatMessage(message, danceClient, danceClient.getCharacter().getName(), ChatType.ROOM);
        this.server.getChatManager().handleMessage(chatMessage);

        if (chatMessage.isDeliver()) {
            SendPacket answerPacket = new SendPacket(PacketType.ROOM_RESPONSE_MESSAGE_ROOM);
            ...
            answerPacket.addByte(0);
            danceClient.getRoom().sendPacket(answerPacket);
        }
