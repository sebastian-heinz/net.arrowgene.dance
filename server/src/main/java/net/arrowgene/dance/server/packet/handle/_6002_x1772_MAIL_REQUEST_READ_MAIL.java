...
public class _6002_x1772_MAIL_REQUEST_READ_MAIL extends HandlerBase {


    public _6002_x1772_MAIL_REQUEST_READ_MAIL(DanceServer server) {
        super(server);
    }
    ...
    public SendPacket[] handle(ReadPacket packet, DanceClient client) {
        ...
        Packet answerPacket;
        if (mail != null && mail.getGiftItemId() > 0) {
            ShopItem gift = super.server.getShop().getShopItem(mail.getGiftItemId());
            answerPacket = MailPacket.getInstance().getReadMailPacket(mail, gift);
        } else {
            answerPacket = MailPacket.getInstance().getReadMailPacket(mail);
        }
