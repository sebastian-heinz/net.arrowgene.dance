...
public class MariaDbSong {

    private MariaDbController controller;
    private MariaDbFactory factory;

    public MariaDbSong(MariaDbController controller, MariaDbFactory factory) {
        this.controller = controller;
        this.factory = factory;
    }

    public void insertSong(Song song) throws SQLException {
        PreparedStatement insert = controller.createPreparedStatement("INSERT OR REPLACE INTO `dance_song` VALUES (?,?,?,?,?,?,?,?,?,?);");
    ...
    public List<Song> getSongs() throws SQLException {
        ...
        PreparedStatement select = controller.createPreparedStatement("SELECT * FROM `dance_song`;");
        ResultSet res = select.executeQuery();
        while (res.next()) {
            Song song = factory.createSong(res);
            songs.add(song);
        }
        res.close();
        select.close();
    ...
    public void deleteSong(int songId) throws SQLException {
        PreparedStatement delete = controller.createPreparedStatement("DELETE FROM `dance_song` WHERE `id`=?;");
